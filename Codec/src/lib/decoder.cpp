...
int decode(int argc, char *argv[])
{
    try
    {
        ...
        for(unsigned count = 1; ; count++)
        {
            ...
            byteArraySerializer.deserializeByteArray(in, &macroblockIsInterPred[0], macroblockIsInterPred.size(), false);
            byteArraySerializer.deserializeByteArray(in, &macroblockIsIntraPred[0], macroblockIsIntraPred.size(), false);
