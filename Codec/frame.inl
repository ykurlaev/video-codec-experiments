...
template <uint32_t N>
template <typename T>
inline bool Frame<N>::BaseIterator<T>::operator==(const BaseIterator<T>& other) const
{
...
template <uint32_t N>
template <typename T>
inline typename Frame<N>::template BaseIterator<T>::this_t Frame<N>::BaseIterator<T>::operator++(int)
{
//###    typename BaseIterator<T>::this_t result(*static_cast<this_t *>(this));
//###    static_cast<typename BaseIterator<T>::this_t *>(this)->operator++();
//###    return result;
}
...
inline typename Frame<N>::Iterator &Frame<N>::Iterator::operator++()
{
    ...
    else if(m_x % BLOCK_SIZE == 0)
    {
        this->m_ptr += BLOCK_SIZE * (BLOCK_SIZE - 1);
    }
    return *this;
}
...
{
    m_scanPtr++;
    if(m_scanPtr - m_scan == BLOCK_SIZE * BLOCK_SIZE)
    {
        m_scanPtr = m_scan;
        m_block++;
        if(m_block == m_count)
        ...
        {
            m_origin += BLOCK_SIZE * BLOCK_SIZE;
            this->m_ptr = m_origin;
        }
    }
    else
